...
const Edit = () => {
  ...
  useEffect(() => {
    ...
    const productToUpdate = storedData.productToUpdate || {};

    setProductId(productToUpdate._id || null);
    setCategoryName(productToUpdate.categoryName || '');
    setProductName(productToUpdate.productName || '');
    setProductImage(productToUpdate.productImage || null);
    setNewPrice(productToUpdate.newPrice ?? '');
    setOldPrice(productToUpdate.oldPrice ?? '');
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      await axios.put(`${backedUrl}/api/editproduct/${productId}`, formData, {
      ...
      <form encType='multipart/form-data' className={CSS['from-container']} onSubmit={handleUpdateProduct}>
        <div>
          ...
          <div className={CSS['product-selectimg-div']}>
            ...
            <input className='' type='file' name='productImage' id='product-img' onChange={(e) => setProductImage(e.target.files[0])} />
          </div>
